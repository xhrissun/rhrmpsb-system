  ...
  properties: {
    ...
    governmentEmployment: {
      ...
      properties: {
        ...
        employmentStartDate: { type: 'STRING', description: 'ISO date (yyyy-mm-dd) if the document states a specific start date for this position. Empty string if not stated. Never estimate or infer a date that is not explicitly written — if only a year is given with no month/day, still leave this empty rather than guessing a month/day.' },
      ...
      required: ['detected', 'agency', 'position', 'status', 'isOngoing', 'employmentStartDate', 'employmentEndDate', 'evidence']
    }
  },
...
2. Training: Only count workshops, trainings, seminars, and capacity-building programs the candidate COMPLETED AS A PARTICIPANT/ATTENDEE. Do NOT count orientations (these are informational sessions, not skills training), conferences (these are for information-sharing/networking, not structured skills training, even when multi-day), or engagements where the candidate served as a guest speaker, resource person, or facilitator (delivering training is not the same as receiving it) — if a document describes one of these, leave it out of the training count and, if it's prominent in the documents (e.g. listed alongside real trainings in the PDS), note in a bullet that it was excluded and why, e.g. "- Excluded: Guest Speaker, [event] — not training received." or "- Excluded: [X] Conference — not a structured training." Trainings/courses conducted by ENRA (the DENR's Environment and Natural Resources Academy) are considered top-tier, highly credible training for this agency's purposes — treat an ENRA-conducted course as fully and unambiguously qualifying without additional scrutiny of its legitimacy or relevance, and where useful say so explicitly, e.g. "- [Course name] (40 hrs) — per Certificate, ENRA-conducted (top-tier)." The PDS's own "Learning and Development Interventions/Training Programs Attended" section (typically page 3 of the CS Form 212) independently lists trainings with their inclusive dates and number of hours — treat it as an equally primary source alongside the standalone Certificates, not a last resort. If Certificates is listed as unavailable, unreadable, or missing (see the note on documents that could not be used as evidence above), rely on the PDS's training section as the fallback source for the training comment instead of treating training evidence as absent — the PDS often lists the exact same trainings, complete with stated hours, independently of whether a separate certificate scan came through. List each qualifying relevant training/seminar found (PDS training section, Certificates) individually with its number of training hours in parentheses immediately after the name, e.g. "- Basic Occupational Safety and Health (40 hrs) — per PDS." If an extracted document states the hours, always include that number this way — never drop it. If a training's hours are NOT stated anywhere but its inclusive start and end dates ARE given, estimate the hours as (number of weekdays, Mon-Fri, spanned by those dates, inclusive) × 8, and label it clearly as an estimate, e.g. "- Basic OSH Training (est. 24 hrs — inferred from 3 weekdays, Nov 3-5, 2023; verify actual hours with certificate)." Only apply this estimate when the span is short and consistent with an in-person/synchronous event (10 calendar days or fewer) — do NOT apply it to a course described as self-paced, asynchronous, online/e-learning, or one spanning more than 10 calendar days (these often run far fewer actual hours than their calendar span, e.g. a MOOC listed as "16 June - 11 July" is not 26 days of training); for those, if hours truly aren't stated anywhere, write "(hours not stated — verify with certificate)" instead, as an explicit warning, same as when no dates are given at all. Then state whether the total relevant hours (stated + estimated) meet the QS training requirement, noting separately if the total only meets it once estimated hours are included. Where a training clearly relates to one of the required competencies above, name that competency.
